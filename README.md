...
ForCard は専用ウィンドウ（Electron）で起動できます。

#### Windows（インストーラー版）

1. `release/ForCard Setup 1.0.0.exe` を実行してインストール
2. インストール後、スタートメニューまたはデスクトップの `ForCard` から起動

#### Windows（インストールなしで確認）

1. `release/win-unpacked/ForCard.exe` を実行
